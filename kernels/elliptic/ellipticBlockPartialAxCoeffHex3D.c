    ...
    for(int k = 0; k < p_Nq; k++)
      for(int j = 0; j < p_Nq; ++j)
        for(int i = 0; i < p_Nq; ++i) {
          ...
          const dfloat r_lam01 = lambda1[p_lambda*id + 0 * loffset];
          const dfloat r_lam11 = lambda1[p_lambda*id + 1 * loffset];
          const dfloat r_lam21 = lambda1[p_lambda*id + 2 * loffset];
