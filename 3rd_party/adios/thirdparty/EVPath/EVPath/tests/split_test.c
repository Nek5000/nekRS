...
/*	    conn = CMinitiate_conn(cm, contact_list);*/
	...
	EVassoc_terminal_action(cm, stone, simple_format_list, (EVSimpleHandlerFunc)simple_handler, NULL);
	...
	if (argc == 2) {
		...
		mid_row_stones[i] = EVcreate_split_action(cm, &list[0]);
	    }
	    for (i=0; i < 2; i++) {
		list[0] = mid_row_stones[i*2];
		list[1] = mid_row_stones[i*2 + 1];
		first_row_stones[i] = EVcreate_split_action(cm, &list[0]);
	    }
	    list[0] = first_row_stones[0];
	    list[1] = first_row_stones[1];
	    stone = EVcreate_split_action(cm, &list[0]);
	} else {
...
{
    fprintf(stderr, "split_test failed to complete in reasonable time\n");
    if (subproc_proc != 0) {
	kill(subproc_proc, 9);
    }
    exit(1);
...
{
    CManager cm;
    char *args[] = {"split_test", "-c", NULL, NULL};
    int exit_state;
    int forked = 0;
    attr_list contact_list, listen_list = NULL;
...
#define HELLO_GROUP "225.0.0.37"
    ...
    EVassoc_terminal_action(cm, handle, simple_format_list, (EVSimpleHandlerFunc)simple_handler, &message_count);
...
/* stuff */
    ...
    CManager_close(cm);
    if (message_count != 8) printf("Message count == %d\n", message_count);
