...
int inverse(double *x,int level,int iter,GenmapHandle h,GenmapComm c,
  mgData d)
{
  assert(level<d->nlevels-2);

  int lelt=d->level_off[1]-d->level_off[0];
  GenmapScalar *y; GenmapMalloc(lelt,&y);
  ...
  int iter,int verbose,GenmapScalar *y)
{
  ...
  GenmapScalar *r; GenmapCalloc(size,&r);
  ...
  for(lvl=nlevels-3; lvl>=0; lvl--){
    ...
    mgLevel l=lvls[lvl]; off=lvl_off[lvl];
