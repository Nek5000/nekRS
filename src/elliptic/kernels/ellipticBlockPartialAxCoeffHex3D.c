#if p_knl == 0
extern "C" void FUNC(ellipticBlockPartialAxCoeffHex3D_v0)(const dlong & Nelements,
                                const dlong & offset,
                                const dlong & loffset,
    ...
    for(int k = 0; k < p_Nq; ++k)
      for(int j = 0; j < p_Nq; ++j)
        for(int i = 0; i < p_Nq; ++i) {
          ...
          const dfloat r_lam00 = lambda0[p_lambda*id + 0 * loffset];
          const dfloat r_lam10 = lambda0[p_lambda*id + 1 * loffset];
          const dfloat r_lam20 = lambda0[p_lambda*id + 2 * loffset];
...
#ifndef p_poisson
          dfloat r_Aq0 = r_GwJ * lambda1[p_lambda*id + 0 * loffset] * s_q[0][k][j][i];
          dfloat r_Aq1 = r_GwJ * lambda1[p_lambda*id + 1 * loffset] * s_q[1][k][j][i];
          dfloat r_Aq2 = r_GwJ * lambda1[p_lambda*id + 2 * loffset] * s_q[2][k][j][i];
...
#else
          Aq[id + 0 * offset] = r_Aqr0 + r_Aqs0 + r_Aqt0;
          Aq[id + 1 * offset] = r_Aqr1 + r_Aqs1 + r_Aqt1;
          Aq[id + 2 * offset] = r_Aqr2 + r_Aqs2 + r_Aqt2;
#endif
        }
  }
}
