program TestBPWriteReadHeatMap4D
  ...
  if (irank == 0) then
    ...
    if (sum_i1 /= 10000*isize) then
       write(*,*) 'Test failed integer*1'
       stop 1
    end if
    if (sum_i2 /= 10000*isize) then
       write(*,*) 'Test failed integer*2'
       stop 1
    end if
    if (sum(sel_temperatures_i4) /= 10000*isize) then
       write(*,*) 'Test failed integer*4'
       stop 1
    end if
    if (sum(sel_temperatures_i8) /= 10000*isize) then
       write(*,*) 'Test failed integer*8'
       stop 1
    end if
    if (sum(sel_temperatures_r4) /= 10000*isize) then
       write(*,*) 'Test failed real*4'
       stop 1
    end if
    if (sum(sel_temperatures_r8) /= 10000*isize) then
