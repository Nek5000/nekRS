...
This tutorial assumes that you have installed *Nek5000* in your home directory and have setup your :ref:`execution PATH<sec:PATH>`.
...
:tip:
  If either ``genbox`` or ``genmap`` cannot be located by your shell, check to make sure the ``Nek5000/tools`` directory is in your path. For help see :ref:`here<sec:PATH>`.
...
The required values for the initial and boundary conditions specified by lower case letters in the  ``.box`` file are defined here as a list of user given parameters, as well as the height of the channel. 
...
For more information on the ``.usr`` file and the available subroutines see :ref:`here <case_files_usr>`.
