...
int read_co2_mesh(Mesh *mesh_,char *fname,struct comm *c){
  ...
  mesh_init(mesh_,nelt,nDim); Mesh mesh=*mesh_;
  mesh->nelgt=nelgt;
  mesh->nelgv=nelgv;
  mesh->nelt=nelt;
  mesh->nDim=nDim;
  ...
  int i,j,tmp1,tmp2;
  for(i=0;i<nelt;i++){
    READ_T(&tmp1,buf0,int,1); buf0+=sizeof(int);
    for(j=0;j<nVertex;j++){
      ptr[i].vertex[j].elementId=tmp1;
      READ_T(&tmp2,buf0,int,1); buf0+=sizeof(int);
...
int write_connectivity(Mesh mesh,char *fileName,struct comm *c){
  ...
  int header_size=GC_CO2_HEADER_LEN+sizeof(float);
  if(rank==0) writeSize+=header_size;
  ...
  for(i=0;i<nelt;i++){
    ...
    WRITE_INT(buf0,temp); buf0+=sizeof(int);
#if defined(GENMAP_DEBUG)
    printf("%d ",temp);
#endif
    for(k=0;k<nVertex;k++){
