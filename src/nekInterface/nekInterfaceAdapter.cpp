                ...
                int NSfields)
{
  ...
  if(coords){
    nrs->cds->meshT[0]->o_x.copyTo(nekData.xm1, Nlocal * sizeof(dfloat));
    nrs->cds->meshT[0]->o_y.copyTo(nekData.ym1, Nlocal * sizeof(dfloat));
    nrs->cds->meshT[0]->o_z.copyTo(nekData.zm1, Nlocal * sizeof(dfloat));
    xo = 1;
  }
  if(o_u.ptr()) {
  ...
  if(o_s.ptr()) {
    ...
    for(int is = 0; is < NSfields; is++) {
      mesh_t* mesh;
      (is) ? mesh = nrs->meshV: mesh = nrs->cds->meshT[0];
...
void copyToNek(dfloat time)
{
  ...
  *(nekData.dp0thdt) = nrs->dp0thdt;
  ...
  if(nrs->Nscalar) {
    if(nrs->options.compareArgs("LOWMACH", "TRUE")) memcpy(nekData.qtl, nrs->div, sizeof(dfloat) * Nlocal);
    const dlong nekFieldOffset = nekData.lelt * mesh->Np;
    for(int is = 0; is < nrs->Nscalar; is++) {
      ...
      const dlong Nlocal = mesh->Nelements * mesh->Np;
      dfloat* Ti = nekData.t   + is * nekFieldOffset;
      dfloat* Si = nrs->cds->S + is * nrs->cds->fieldOffset[0];
