...
int readRe2Boundaries(Mesh mesh,MPI_File file,struct comm *c){
  ...
  double ncurvesD; READ_T(&ncurvesD,bufL,long,1);
  ...
  double nbcsD; READ_T(&nbcsD,bufL,long,1); long nbcs=nbcsD;
  ...
  for(i=0;i<nbcsLocal;i++){
    READ_T(tmp,buf0,long,1);buf0+=sizeof(long);
    boundary.elementId=tmp[0]-1;

    READ_T(tmp,buf0,long,1);buf0+=sizeof(long);
    boundary.faceId=PRE_TO_SYM_FACE[(long)tmp[0]-1];

    READ_T(tmp,buf0,long,5);buf0+=5*sizeof(long);
    READ_T(cbc,buf0,char,3);buf0+=sizeof(long);
    cbc[3]='\0';
