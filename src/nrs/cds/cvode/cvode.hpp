...
public:
  ...
  using userRHS_t = std::function<
      void(double time, double t0, const LVector_t<dfloat> &o_y, LVector_t<dfloat> &o_ydot)>;
  using userJacobian_t = std::function<
      void(double time, double t0, const LVector_t<dfloat> &o_y, LVector_t<dfloat> &o_ydot)>;
  using userLocalPointSourceL_t =
      std::function<void(double time, const LVector_t<dfloat> &o_y, LVector_t<dfloat> &o_ydot)>;
  using userLocalPointSourceE_t =
      std::function<void(double time, const deviceMemory<dfloat> &o_y, deviceMemory<dfloat> &_ydot)>;

  using userPostNrsToCv_t = std::function<void(LVector_t<dfloat> &o_LField, bool isYdot)>;
  using userPostCvToNrs_t = std::function<void(deviceMemory<dfloat> &o_EField, bool isYdot)>;
  using userMakeq_t = std::function<void(double time)>;
  using userPreSolve_t = std::function<void()>;
  using userPostSolve_t = std::function<void()>;

  using evaluateProperties_t = std::function<void(double)>;
  using evaluateDivergence_t = std::function<void(double)>;
  ...
  void setEvaluateProperties(evaluateProperties_t _evaluateProperties)
  {
    evaluateProperties = _evaluateProperties;
  }

  void setEvaluateDivergence(evaluateDivergence_t _evaluateDivergence)
  {
    evaluateDivergence = _evaluateDivergence;
  }
