...
char *submit_spec = "{\n\
	int ret = input.long_field % 2;\n\
        if (ret) {\n\
	    simple tmp;\n\
    long sum = 0;\n\
    tmp.integer_field = (int) lrand48() % 100;\n\
    sum = sum + tmp.integer_field % 100;\n\
    tmp.short_field = ((short) lrand48());\n\
    sum = sum + tmp.short_field % 100;\n\
    tmp.long_field = ((long) lrand48());\n\
    sum = sum + tmp.long_field % 100;\n\
\n\
    tmp.nested_field.item.r = drand48();\n\
    sum = sum + ((int) (tmp.nested_field.item.r * 100.0)) % 100;\n\
    tmp.nested_field.item.i = drand48();\n\
    sum = sum + ((int) (tmp.nested_field.item.i * 100.0)) % 100;\n\
\n\
    tmp.double_field = drand48();\n\
    sum = sum + ((int) (tmp.double_field * 100.0)) % 100;\n\
    tmp.char_field = lrand48() % 128;\n\
    sum = sum + tmp.char_field;\n\
    sum = sum % 100;\n\
    tmp.scan_sum = (int) sum;\n\
	    EVsubmit(0, tmp);\n\
	}\n\
	return 0;\n\
}\0\0";
...
/*	    conn = CMinitiate_conn(cm, contact_list);*/
	...
	filter = create_filter_action_spec(filter_format_list, submit_spec);
...
{
    CManager cm;
    char *args[] = {"submit_test", "-c", NULL, NULL};
    char *filter;
    int exit_state;
    int forked = 0;
    attr_list contact_list, listen_list = NULL;
