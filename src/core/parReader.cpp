    ...
    if(usesAVM){
      for(std::string s : list){
        ...
        if(s.find("sensitivity") == 0)
        {
          std::vector<string> items = serializeString(s, '=');
          assert(items.size() == 2);
          const dfloat sensorSens = std::stod(items[1]);
          options.setArgs(parPrefix + "SENSITIVITY", to_string_f(sensorSens));
        }
        ...
        if(s.find("sensororder") == 0)
        {
          std::vector<string> items = serializeString(s, '=');
          assert(items.size() == 2);
          const dfloat sensorOrder = std::stod(items[1]);
          options.setArgs(parPrefix + "SENSOR ORDER", to_string_f(sensorOrder));
        }
      }
    }
